...
\end{align}
where $T_i(u)$, $i=1,\dots,k_u$, are the interarrival times of the threshold $u$ exceedances and $k_u$ is the number of exceedances, we get the following estimator for $\theta$ depending on $\beta$:
...
\\
Therefore, we choose the \emph{Max-autoregressive (MAR) Process} with extremal index $\theta \in (0,1)$ for $(J_i)_{i \in \mathbb{N}}$ which is defined by
\begin{align}
J_1&:=X_1/\theta \\
...
\subsection{Probleme}
\begin{enumerate}
	\item es gibt tatsächlich keine Wartezeiten, die gleich Null sind -> daher gilt nicht $\lim\limits_{p \to 0} m_p = \theta$. Sondern $\lim\limits_{p \to 0} m_p = 1$. 
\end{enumerate}
\end{document}